...
- **多媒体**：通过 AstrBot 分别选择图片、音频模型，描述图片、识别文字和转写音频；视频和普通文件保留占位符。
- **成本控制**：按群／私聊选择媒体策略，限制输入规模与每日额度；支持成功缓存、逐附件手动处理和节省预设。
- **用量统计**：查看插件调用的 token 趋势、缓存输入、输出及逐次明细，按会话、提供商和任务筛选。
...
3. 需要图片或语音识别时，分别选择处理模型，并在 AstrBot 提供商中勾选实际支持的「图像」或「音频」能力；留空继承后台模型。
4. 聊天后发送 `/memoir` 查看统计，或从插件详情打开「记忆管理」。

默认开启「自动兼容 AstrBot」：原生 STT、图片转述或群聊上下文接管时，停用对应插件处理，减少重复消耗；在会话设置查看生效状态。转发内部媒体仍独立处理，详见[兼容规则](docs/guide.md#astrbot-原生功能兼容)。
