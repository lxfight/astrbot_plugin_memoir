...
- **原文对话**：搜索包含转发分块的正文，可按发送者、日期、来源和处理状态筛选。列表按原始事件分页；展开转发可查看全部现存分块、折叠层级、复制全文或删除单个分块。来源详情中的返回按钮保留原列表位置。
- **批量删除**：点击「选择记录」后选择当前页，确认范围再删除。删除记忆不会删除原文；删除转发事件会连同分块和解析任务删除，已提炼记忆保留。清空会话才会一并清除。
